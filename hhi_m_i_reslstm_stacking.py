# -*- coding: utf-8 -*-
"""HHI_M_I_RESLSTM_STACKING.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/18FsQXYWJLCIRIRg9AwvOjfJFLuBFrE40
...
fitted = fitted.values # np.exp(predictions) # *div[-1]
values = dohody[-12:].values
...
def train_wrapper(config):
  ...
  if result_metrics["mae"] < mae:
    print("OLD MAE:", mae)
    torch.save(opt.model.state_dict(), "/content/hhi_best_LSTM.pt")
    f = open("/content/bestmae.txt", "w")
...
"""**Best trial config: {'learning_rate': 0.001, 'batch_size': 16, 'input_dim': 24, 'hidden_dim': 128, 'layer_dim': 3, 'dropout': 0.1, 'delta': 1.0}**


Best trial final validation accuracy (MAE): 2.75495
...
Lmodel.load_state_dict(torch.load("/content/hhi_best_LSTM.pt"))
Lmodel.eval()

Lopt = Trainer(model=Lmodel, loss_fn=None, optimizer=None)

X, y, div = createDataset(df, 24, 12)
X_train, X_val, X_test, y_train, y_val, y_test = train_val_test_split(X, y, 0.2)

Lpredictions, values = Lopt.evaluateFromArrays(X_test, y_test, n_features=24)
...
def train_wrapper(config):
  ...
  if result_metrics["mae"] < mae:
    print("OLD MAE:", mae)
    torch.save(opt.model.state_dict(), "/content/hhi_best_GRU.pt")
    f = open("/content/bestmae.txt", "w")
    f.write(str(result_metrics["mae"]))
    f.close()
  tune.report(accuracy=result_metrics["mae"])
...
Gmodel.load_state_dict(torch.load("/content/hhi_best_GRU.pt"))
Gmodel.eval()
...
Gpredictions, values = Gopt.evaluateFromArrays(X_test, y_test, n_features=12)
...
def train_wrapper(config):
  ...
  if result_metrics["mae"] < mae:
    print("OLD MAE:", mae)
    torch.save(opt.model.state_dict(), "/content/hhi_best_ResLSTM.pt")
    f = open("/content/bestmae.txt", "w")
    f.write(str(result_metrics["mae"]))
    f.close()
  tune.report(accuracy=result_metrics["mae"])
...
RLmodel.load_state_dict(torch.load("/content/hhi_best_ResLSTM.pt"))
RLmodel.eval()
...
RLpredictions, values = RLopt.evaluateFromArrays(X_test, y_test, n_features=36)
...
yt = [2.75495, 3.00226, 3.06793, 6.61068, 3.77361]
