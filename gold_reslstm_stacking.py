# -*- coding: utf-8 -*-
"""Gold_RESLSTM_STACKING.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1n44589kA8letykYGy66OYqWe38TUbzwT
...
def train_wrapper(config):
  ...
  if result_metrics["mae"] < mae:
    print("OLD MAE:", mae)
    torch.save(opt.model.state_dict(), "/content/best_LSTM.pt")
    f = open("/content/bestmae.txt", "w")
...
Lmodel.load_state_dict(torch.load("/content/gold_best_LSTM.pt"))
Lmodel.eval()
...
Lpredictions, values = Lopt.evaluateFromArrays(X_test, y_test, n_features=12)
...
def train_wrapper(config):
  ...
  if result_metrics["mae"] < mae:
    print("OLD MAE:", mae)
    torch.save(opt.model.state_dict(), "/content/gold_best_GRU.pt")
    f = open("/content/bestmae.txt", "w")
    f.write(str(result_metrics["mae"]))
    f.close()
  tune.report(accuracy=result_metrics["mae"])
...
Gmodel.load_state_dict(torch.load("/content/gold_best_GRU.pt"))
Gmodel.eval()
...
def train_wrapper(config):
  ...
  if result_metrics["mae"] < mae:
    print("OLD MAE:", mae)
    torch.save(opt.model.state_dict(), "/content/gold_best_ResLSTM.pt")
    f = open("/content/bestmae.txt", "w")
    f.write(str(result_metrics["mae"]))
    f.close()
  tune.report(accuracy=result_metrics["mae"])
...
RLmodel.load_state_dict(torch.load("/content/gold_best_ResLSTM.pt"))
RLmodel.eval()
...
RLpredictions, values = RLopt.evaluateFromArrays(X_test, y_test, n_features=24)
...
layer_props = (128, 4)
for i in range(4):
  hidden_dim, layer_dim = layer_props
  model_params = {
    'input_dim': input_dim,
...
yt = [ 75.14617, 29.9919, 19.99713, 119.039, 68.9254]
